...
All images are local and committed — nothing is loaded from an external host: `public/imagens` (hero portrait `hero.png`, grupos background `grupos-fundo.png`, share preview `og.png` at 1200×600, logos/SVGs), `public/bandeiras` (one country-flag SVG per ISO code, e.g. `br.svg`, used by `BandeiraPais`; `AC` and `TA` have no file and fall back to the ISO letters), plus the favicon set in `public/`. The favicon set (`icon.svg`, `favicon.ico`, `favicon-{16,32,48}x{16,32,48}.png`, `apple-touch-icon.png`, `android-chrome-*.png` incl. a maskable one, `mstile-150x150.png`) is wired up in the `metadata` export of `src/app/layout.jsx`, as are the Open Graph/Twitter preview tags. `og:image` must be an absolute URL, so `generateMetadata` in `src/app/layout.jsx` builds `metadataBase` per request from `NEXT_PUBLIC_SITE_URL` (optional override, see `.env.example`), else the request's `X-Forwarded-Host`/`Host` + `X-Forwarded-Proto` — there is deliberately no hardcoded domain (this makes `/` render on demand instead of static). Note `next dev` always resolves social images against `http://localhost:<PORT>` (Next hardcodes that in development), so the real host only shows up under `next build` + `next start`. The PWA manifest is `src/app/manifest.js`. The master artwork is `public/imagens/icone.svg` (489×512); `public/icon.svg` is the same drawing with a square viewBox, and every PNG/ICO was rasterized from it (transparent for browser tabs, white background for iOS/Android/Windows tiles).
...
- `atomos/` — self-contained primitives with no dependency on other components: Botao, Campo, Etiqueta, Icones (inline SVGs; the 8 proposal-card icons are Phosphor "regular" paths on a 256 viewBox, the rest are hand-drawn 24px line icons, no icon package is installed), Logo, Revelar, BandeiraPais (local SVG from `public/bandeiras`, falls back to the ISO code), BandeiraEstado, MarcaParceiro.
